...
contents, archive reproducibility, and a full-router replay against the bundled hash-pinned
`public-demo-v1` synthetic registry. The public demo also proves that a demo profile cannot claim a
Git commit or escape its fixed fixture scope.
...
and is not shipped here. The public synthetic replay exercises the same router mechanics but grants
no lifecycle authority and is not a substitute for the owner-bound replay.
